...
//    @BeforeEach
    ...
    void findOneTest() {
        ...
        Employee employee = employeeRepository
                .findById(id)
                .orElseThrow(// orElseThrow : find결과가 null일 경우 예외가 발생하도록 예외 처리
                        () -> new RuntimeException("사원이 조회되지 아니함.")
                );
        ...
        /*
            select쿼리가 안나갔지만 조회되는 이유
            영속성 컨텍스트라는 개념이 JPA에 있어서
            INSERT 직후 SELECT를 하면 두개의 쿼리가 하나의 트랜잭션으로 연결되어
            INSERT한 내용을 메모리에서 불러오기 때문에 DB를 조회하지 않고 성능 최적화를 함.
        */
    }
